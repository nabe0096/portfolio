...
window.__build=async function(A){
 ...
 for(let i=1;i<=A.nsteps;i++){
  ...
  let t=0; while(cnt()===b&&t<40){await sleep(500);t++;}
 }
...
window.__finish=async function(dateStr){
  ...
  if(m && /使用できない|不足/.test(m.innerText)){ return {error:m.innerText.slice(0,140)}; }
  ...
  await sleep(6500);
  return {url:location.pathname, result:document.querySelector('.sd-modal.is-shown')?.innerText.slice(0,46)||null};
};
// 予約済み記事の公開日時を変える（/article/update/<id> を開いた状態で呼ぶ）。dateStr='2026/10/05 11:00'
window.__reschedule=async function(dateStr){
  const pub=[...document.querySelectorAll('button')].find(b=>b.innerText.trim()==='公開設定'); if(!pub) return {error:'no pub'};
  pub.click(); await sleep(2500);
  let m=document.querySelector('.sd-modal.is-shown'); if(!m) return {error:'no modal'};
  const chg=[...m.querySelectorAll('button')].find(b=>b.innerText.trim()==='予約日時を変更する');
  if(!chg) return {error:'not scheduled: '+m.innerText.slice(0,60)};
  chg.click(); await sleep(2500);
  const m2=document.querySelector('.sd-modal.is-shown');
  const inp=m2.querySelector('input[type=text]');
  Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value').set.call(inp,dateStr);
  inp.dispatchEvent(new Event('input',{bubbles:true})); inp.dispatchEvent(new Event('change',{bubbles:true})); inp.blur();
  await sleep(1000);
  const btn=[...m2.querySelectorAll('button')].find(b=>b.innerText.trim()==='公開予約');
  if(!btn||btn.disabled) return {error:'button disabled'};
  btn.click(); await sleep(6500);
  return {url:location.pathname, result:document.querySelector('.sd-modal.is-shown')?.innerText.slice(0,60)||null};
};
'lib-ready'
